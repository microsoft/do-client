...
TEST_F(DownloadPropertyTestsDOSVC, SmallDownloadWithPhfDigestandCvTest)
{
    ...
    msdo::download_property_value integrityCheckMandatory;
    integrityCheckMandatory.make(true);
    ...
    msdo::download_property_value integrityCheckInfo;
    integrityCheckInfo.make(g_smallFilePhfInfoJson);
    VerifyCallWithExpectedErrors([&]()
        {
            simpleDownload.set_property(msdo::download_property::integrity_check_info, integrityCheckInfo);
        }, expectedErrors);

    msdo::download_property_value correlationVector;
    std::string strCorrelationVector("g+Vo71JZwkmJdYfF.0");
    correlationVector.make(strCorrelationVector);
    VerifyCallWithExpectedErrors([&]()
        {
...
TEST_F(DownloadPropertyTestsDOSVC, SmallDownloadWithCustomHeaders)
{
    ...
    msdo::download_property_value httpCustomHeaders;
    std::string strHttpCustomHeaders("XCustom1=someData\nXCustom2=moreData");
    httpCustomHeaders.make(strHttpCustomHeaders);
...
TEST_F(DownloadPropertyTestsDOSVC, CallbackTestUseDownload)
{
    ...
    msdo::download_property_value callback;
    callback.make([&hitError](msdo::download& download, msdo::download_status& status)
    {
        char msgBuf[1024];
...
TEST_F(DownloadPropertyTestsDOSVC, SetCallbackTest)
{
    ...
    int i= 0;
    msdo::download_property_value callback;
    callback.make([&i](msdo::download& download, msdo::download_status& status)
        {
            char msgBuf[1024];
            snprintf(msgBuf, sizeof(msgBuf), "Received status callback: %llu/%llu, 0x%x, 0x%x, %u",
                status.bytes_transferred(), status.bytes_total(), status.error_code(), status.extended_error_code(),
...
TEST_F(DownloadPropertyTestsDOSVC, OverrideCallbackTest)
{
    ...
    int i = 0;
    msdo::download_property_value callback;
    callback.make([&i](msdo::download&, msdo::download_status&)
        {
            i += 1;
        });
    simpleDownload.set_property(msdo::download_property::callback_interface, callback);
    ...
    double backgroundDuration = TimeOperation([&]()
        {
            ...
            msdo::download_property_value foregroundPriority;
            foregroundPriority.make(false);
