...
int32_t download::start_and_wait_until_completion_nothrow(const std::atomic_bool& isCancelled, std::chrono::seconds timeOut) noexcept
{
    ...
    RETURN_RES_IF_FAILED(start_nothrow());
    download_status status;

    RETURN_RES_IF_FAILED(get_status_nothrow(status));
    ...
    {
        RETURN_RES_IF_FAILED(finalize_nothrow());
    }
    else
    {
        RETURN_RES_IF_FAILED(abort_nothrow());
        if (isCancelled)
        {
