...
#else
    // If an application builds the sdk from source and has toggled DO_DISABLE_EXCEPTIONS, it would be hypocritical for their callback to throw
...
int32_t CDownloadImpl::_Initialize(const std::string& uri, const std::string& downloadFilePath) noexcept
{
    ...
    RETURN_RES_IF_FAILED(CoCreateInstance(__uuidof(DeliveryOptimization), nullptr, CLSCTX_LOCAL_SERVER, IID_PPV_ARGS(&manager)));
    Microsoft::WRL::ComPtr<IDODownload> spDownload;
    RETURN_RES_IF_FAILED(manager->CreateDownload(&spDownload));

    RETURN_RES_IF_FAILED(CoSetProxyBlanket(static_cast<IUnknown*>(spDownload.Get()), RPC_C_AUTHN_DEFAULT,
    ...
    RETURN_RES_IF_FAILED(_SetPropertyHelper(*spDownload.Get(), download_property::uri, propUri));
    RETURN_RES_IF_FAILED(_SetPropertyHelper(*spDownload.Get(), download_property::download_file_path, propDownloadFilePath));
...
int32_t CDownloadImpl::GetStatus(msdo::download_status& status) noexcept
{
    ...
    RETURN_RES_IF_FAILED(_spDownload->GetStatus(&comStatus));
...
int32_t CDownloadImpl::SetCallback(const download_property_value::status_callback_t& callback, download& download) noexcept
{
    ...
    RETURN_RES_IF_FAILED(MakeAndInitialize<DOStatusCallback>(&spCallback, callback, download));
    ...
    RETURN_RES_IF_FAILED(ConvertToComProperty(msdo::download_property::callback_interface, prop));
    const auto res = _spDownload->SetProperty(prop, &vtCallback);
...
{
    DODownloadProperty prop;
    RETURN_RES_IF_FAILED(ConvertToComProperty(key, prop));

    std::shared_ptr<CDownloadPropertyValueInternal> prop_val = val._val;
    // COM API SetProperty() is not declared with const reference for variant argument so need to const_cast
    return download.SetProperty(prop, &const_cast<CDownloadPropertyValueInternal::native_type&>(prop_val->native_value()));
}
